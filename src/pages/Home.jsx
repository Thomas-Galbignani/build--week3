...
export default function Home({ token }) {
    ...
    useEffect(() => {
        ...
        const fetchMe = async () => {
            try {
                ...
                if (!myId) return;
                const res = await fetch(`https://striveschool-api.herokuapp.com/api/profile/${myId}`, { headers: { Authorization: `Bearer ${token}` } });
                if (!res.ok) return;
    ...
    return (
        <Container className="py-3">
            <Row className="g-4">
                <Col xs={12} lg={3}>
                    <Card className="shadow-sm overflow-hidden position-relative p-0" style={{ margin: 0 }}>
                        <div className="position-relative" style={{ height: "50%" }}>
                            <img src="https://www.20i.com/blog/wp-content/uploads/2021/04/burst.jpg" alt="cover" className="w-100 h-100" style={{ objectFit: "cover" }} />
                            <div className="position-absolute" style={{ left: 16, bottom: -36 }}>
                                {me?.image ? (
                                    <img src={me.image} alt="Tu" width={72} height={72} className="rounded-circle border border-2 border-white" style={{ objectFit: "cover" }} />
                                ) : (
                                    <div
                        ...
                        <div className="px-3 pb-3 text-start" style={{ padding: "48px 12px 12px 96px" }}>
                            <div className="fw-semibold">{me ? `${me.name} ${me.surname}` : "Tu"}</div>
                ...
                <Col xs={12} lg={6}>
                    <NewPostForm token={token} onCreated={(created) => setPosts((prev) => [created, ...prev])} userImage={me?.image} userName={me ? `${me.name} ${me.surname}` : "Tu"} />
                    <Posts
                        posts={posts}
